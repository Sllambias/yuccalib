...
def create_zero_centered_coordinate_matrix(shape):
    if len(shape) == 3:
        mesh = np.array(np.meshgrid(np.arange(shape[0]), np.arange(shape[1]), np.arange(shape[2]), indexing='ij')).astype(float)
    if len(shape) == 2:
...
def deform_coordinate_matrix(coordinate_matrix, alpha, sigma):
    deforms = np.array([gaussian_filter((np.random.random(coordinate_matrix.shape[1:]) * 2 - 1), sigma, mode="constant", cval=0) * alpha
...
def get_max_rotated_size(patch_size):
    ...
    max_dim_0 = max(int(np.sqrt(patch_size[0]**2 + patch_size[1]**2)),
                    int(np.sqrt(patch_size[0]**2 + patch_size[2]**2)))

    max_dim_1 = max(int(np.sqrt(patch_size[1]**2 + patch_size[0]**2)),
                    int(np.sqrt(patch_size[1]**2 + patch_size[2]**2)))

    max_dim_2 = max(int(np.sqrt(patch_size[2]**2 + patch_size[0]**2)),
