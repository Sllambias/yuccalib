...
class AdditiveNoise(YuccaTransform):
    ...
    @staticmethod
    def get_params(mean: Tuple[float],
                   sigma: Tuple[float]) -> Tuple[float]:
        ...
        for b in range(data_dict[self.data_key].shape[0]):
            for c in range(data_dict[self.data_key][b].shape[0]):
                ...
                if np.random.uniform() < self.p_per_sample:
                    data_dict[self.data_key][b, c] = self.__additiveNoise__(data_dict[self.data_key][b, c], 
                mean, sigma)
        return data_dict
    ...
    def __call__(self, **data_dict):
        ...
        for b in range(data_dict[self.data_key].shape[0]):
            for c in range(data_dict[self.data_key][b].shape[0]):
                if np.random.uniform() < self.p_per_sample:
                    mean, sigma = self.get_params(self.mean, self.sigma)
                    data_dict[self.data_key][b, c] = self.__multiplicativeNoise__(data_dict[self.data_key][b, c], 
                    mean, sigma)
        return data_dict