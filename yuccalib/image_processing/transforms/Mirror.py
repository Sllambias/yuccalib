    ...
    def __call__(self, **data_dict):
        ...
        for b in range(data_dict[self.data_key].shape[0]):
            if np.random.uniform() < self.p_per_sample:
                if self.skip_seg:
                    data_dict[self.data_key][b] = self.__mirrorimage__(data_dict[self.data_key][b], 
                                                                       self.axes)
                else:
                    data_dict[self.data_key][b], data_dict[self.seg_key][b] = self.__mirror__(data_dict[self.data_key][b],
