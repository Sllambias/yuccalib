...
class SimulateLowres(YuccaTransform):
    ...
    def __simulatelowres__(self, imageVolume, target_shape):
        ...
        downsampled = resize(imageVolume.astype(float), target_shape, order=0, mode='edge',
                            anti_aliasing=False)
        imageVolume = resize(downsampled, shape, order=3, mode='edge',
                                anti_aliasing=False)
        return imageVolume
            ...
            if np.random.uniform() < self.p_per_sample:
                for c in range(data_dict[self.data_key][b].shape[0]):
                    if np.random.uniform() < self.p_per_channel:
                        target_shape = self.get_params(self.zoom_range, data_dict[self.data_key][b,c].shape, self.p_per_axis)
