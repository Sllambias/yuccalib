...
class MotionGhosting(YuccaTransform):
    ...
    def __init__(self, data_key="image", p_per_sample=1,
                 alpha=(0.85, 0.95), numReps=(2,5), axes=(0,3)):
    ...
    @staticmethod
    def get_params(alpha: Tuple[float],
                   numReps: Tuple[float],
                   axes: Tuple[float]) -> Tuple[float]:
        ...
        for b in range(data_dict[self.data_key].shape[0]):
            for c in range(data_dict[self.data_key][b].shape[0]):
                if np.random.uniform() < self.p_per_sample:
                    ...
                    data_dict[self.data_key][b, c] = self.__motionGhosting__(data_dict[self.data_key][b, c], 
                                                                                alpha, numReps, axis)
        return data_dict  
