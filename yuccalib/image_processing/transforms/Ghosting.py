...
class MotionGhosting(YuccaTransform):
    ...
    @staticmethod
    def get_params(
        alpha: Tuple[float], numReps: Tuple[float], axes: Tuple[float]
    ) -> Tuple[float]:
    ...
    def __motionGhosting__(self, imageVolume, alpha, numReps, axis):
        if len(imageVolume.shape) == 3:
            ...
            if axis == 0:
                imageVolume[0:-1:numReps, :, :] = (
                    alpha * imageVolume[0:-1:numReps, :, :]
                )
            elif axis == 1:
                imageVolume[:, 0:-1:numReps, :] = (
                    alpha * imageVolume[:, 0:-1:numReps, :]
        ...
        for b in range(data_dict[self.data_key].shape[0]):
            for c in range(data_dict[self.data_key][b].shape[0]):
                if np.random.uniform() < self.p_per_sample:
                    alpha, numReps, axis = self.get_params(
                        self.alpha, self.numReps, self.axes
                    )
