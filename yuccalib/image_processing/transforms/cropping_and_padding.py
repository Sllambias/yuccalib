...
class CropPad(YuccaTransform):
    ...
    def __init__(self, data_key = 'image', seg_key = 'seg',
                 patch_size: tuple | list = None, p_oversample_foreground=0.):
    ...
    def __croppad__(self, image: np.ndarray, seg: np.ndarray, image_properties: dict, 
                    target_image_shape: list | tuple, target_seg_shape: list | tuple):
        
        if len(self.patch_size) == 3:
            return self.generate_3D_case_from_3D(image, seg, image_properties, 
                                                 target_image_shape, target_seg_shape)
        elif len(self.patch_size) == 2 and len(self.input_shape) == 4:
            return self.generate_2D_case_from_3D(image, seg, image_properties, 
                                                 target_image_shape, target_seg_shape)
        elif len(self.patch_size) == 2 and len(self.input_shape) == 3:
            return self.generate_2D_case_from_2D(image, seg, image_properties, 
                                                 target_image_shape, target_seg_shape)


    def generate_3D_case_from_3D(self, image, seg, image_properties,
                                 target_image_shape, target_seg_shape):
        ...
        if len(image_properties['foreground_locations']) == 0 or np.random.uniform() >= self.p_oversample_foreground:
            ...
            for d in range(3):
                if image.shape[d+1] < self.patch_size[d]:
                    ...
                    crop_start_idx += [np.random.randint(max(0, location[d] - self.patch_size[d]),
                                                        min(location[d], image.shape[d+1]-self.patch_size[d]) + 1)]

        image_out[:, :, :, :,] = np.pad(image[:,
    ...
    def generate_2D_batch_from_3D(self, image, seg, image_properties,
                                  target_image_shape, target_seg_shape):
        """
        ...
        if len(image_properties['foreground_locations']) == 0 or np.random.uniform() >= self.p_oversample_foreground:
            ...
            for d in range(2):
                if image.shape[d+2] < self.patch_size[d]:
                    ...
                    crop_start_idx += [np.random.randint(max(0, location[d+1] - self.patch_size[d]),
                                                        min(location[d+1], image.shape[d+2]-self.patch_size[d]) + 1)]

        image_out[:, :, :, :] = np.pad(image[:, x_idx,
                                    crop_start_idx[0]: crop_start_idx[0] + self.patch_size[0],
                                    crop_start_idx[1]: crop_start_idx[1] + self.patch_size[1]], 
    ...
    def generate_2D_batch_from_2D(self, image, seg, image_properties,
                                  target_image_shape, target_seg_shape):
        """
        The possible input for this can be 2D or 3D data.
        For 2D we want to pad or crop as necessary.
    ...
    def __call__(self, packed_data_dict = None, image_properties = None, **unpacked_data_dict):
        ...
        target_image_shape, target_seg_shape = self.get_params(data_dict[self.data_key].shape,
                                                                self.patch_size)

        data_dict[self.data_key], data_dict[self.seg_key] = self.__croppad__(
            data_dict[self.data_key], data_dict[self.seg_key], image_properties,
            target_image_shape, target_seg_shape)
        return data_dict
