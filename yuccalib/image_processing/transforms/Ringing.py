...
class GibbsRinging(YuccaTransform):
    ...
    def __init__(
        self, data_key="image", p_per_sample=1, cutFreq=(96, 129), axes=(0, 3)
    ):
    ...
    def __gibbsRinging__(self, imageVolume, numSample, axis):
        if len(imageVolume.shape) == 3:
            ...
            if axis == 0:
                ...
                imageVolume = abs(
                    np.fft.ifftn(np.fft.ifftshift(imageVolume), s=[h, d, w])
                )
                imageVolume = imageVolume.transpose(0, 2, 1)
            elif axis == 1:
                ...
                imageVolume = abs(
                    np.fft.ifftn(np.fft.ifftshift(imageVolume), s=[w, d, h])
                )
                ...
                imageVolume = abs(
                    np.fft.ifftn(np.fft.ifftshift(imageVolume), s=[h, w, d])
                )
            ...
            for c in range(data_dict[self.data_key][b].shape[0]):
                if np.random.uniform() < self.p_per_sample:
                    ...
                    data_dict[self.data_key][b, c] = self.__gibbsRinging__(
                        data_dict[self.data_key][b, c], cutFreq, axis
                    )
        return data_dict
