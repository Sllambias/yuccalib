...
from yuccalib.network_architectures.blocks_and_layers.conv_layers import ConvDropoutNormNonlin, ConvDropoutNorm
from yuccalib.network_architectures.blocks_and_layers.norm import LayerNorm3d
...
class Multiresblock(nn.Module):
    ...
    def __init__(self, num_in_channels, num_filters,
        ...
        self.shortcut = ConvDropoutNorm(num_in_channels, num_out_filters, 
                                        self.conv_op, shortcut_kwargs,
                                        self.norm_op, self.norm_op_kwargs,
                                        self.dropout_op, self.dropout_op_kwargs,
                                        self.nonlin, self.nonlin_kwargs)

        self.conv_3x3 = basic_block(num_in_channels, filt_cnt_3x3, 
                                    self.conv_op, self.conv_kwargs,
                                    self.norm_op, self.norm_op_kwargs,
                                    self.dropout_op, self.dropout_op_kwargs,
                                    self.nonlin, self.nonlin_kwargs)

        self.conv_5x5 = basic_block(filt_cnt_3x3, filt_cnt_5x5,
                                    self.conv_op, self.conv_kwargs,
                                    self.norm_op, self.norm_op_kwargs,
                                    self.dropout_op, self.dropout_op_kwargs,
                                    self.nonlin, self.nonlin_kwargs)

        self.conv_7x7 = basic_block(filt_cnt_5x5, filt_cnt_7x7,
                                    self.conv_op, self.conv_kwargs,
                                    self.norm_op, self.norm_op_kwargs,
                                    self.dropout_op, self.dropout_op_kwargs,
                                    self.nonlin, self.nonlin_kwargs)
...
class Respath(torch.nn.Module):
    ...
    def __init__(self, num_in_filters, num_out_filters, respath_length, 
        ...
        for i in range(self.respath_length):
            if(i==0):
                self.shortcuts.append(ConvDropoutNorm(num_in_filters, num_out_filters,
                                                    self.conv_op, shortcut_kwargs,
                                                    self.norm_op, self.norm_op_kwargs,
                                                    self.dropout_op, self.dropout_op_kwargs,
                                                    self.nonlin, self.nonlin_kwargs))
                self.convs.append(basic_block(num_in_filters, num_out_filters, 
                                            self.conv_op, self.conv_kwargs,
                                            self.norm_op, self.norm_op_kwargs,
                                            self.dropout_op, self.dropout_op_kwargs,
                                            self.nonlin, self.nonlin_kwargs))
                ...
                self.shortcuts.append(ConvDropoutNorm(num_out_filters, num_out_filters,
                                                    self.conv_op, shortcut_kwargs,
                                                    self.norm_op, self.norm_op_kwargs,
                                                    self.dropout_op, self.dropout_op_kwargs,
                                                    self.nonlin, self.nonlin_kwargs))
                
                self.convs.append(basic_block(num_out_filters, num_out_filters,
                                            self.conv_op, self.conv_kwargs,
                                            self.norm_op, self.norm_op_kwargs,
                                            self.dropout_op, self.dropout_op_kwargs,
                                            self.nonlin, self.nonlin_kwargs))
...
class UXNet_encoder(nn.Module):
    ...
    """
    def __init__(self, in_channels=1, depths=[2, 2, 2, 2], dims=[48, 96, 192, 384],
        ...
        for i in range(4):
            stage = nn.Sequential(
                *[self.basic_block(dim=dims[i], conv_op=self.conv_op, norm_op=self.norm_op,
