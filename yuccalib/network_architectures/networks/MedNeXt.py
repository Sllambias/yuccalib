...
class MedNeXt(YuccaNet):
    ...
    def __init__(
        ...
        self.out_0 = OutBlock(
            in_channels=n_channels, n_classes=self.num_classes, dim=dim
        )
        ...
        if self.deep_supervision:
            self.out_1 = OutBlock(
                in_channels=n_channels * 2, n_classes=self.num_classes, dim=dim
            )
            self.out_2 = OutBlock(
                in_channels=n_channels * 4, n_classes=self.num_classes, dim=dim
            )
            self.out_3 = OutBlock(
                in_channels=n_channels * 8, n_classes=self.num_classes, dim=dim
            )
            self.out_4 = OutBlock(
                in_channels=n_channels * 16, n_classes=self.num_classes, dim=dim
            )
