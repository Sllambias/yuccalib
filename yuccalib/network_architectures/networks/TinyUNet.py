...
class TinyUNet(YuccaNet):
    def __init__(self,
        ...
        self.upsample1 = self.upsample(self.filters*4, self.filters*2, kernel_size=2,
                                       stride=2)
        self.decoder_conv1 = self.basic_block(self.filters*4, self.filters*2,
                                              self.conv_op, self.conv_kwargs,
                                              self.norm_op, self.norm_op_kwargs,
                                              self.dropout_op, self.dropout_op_kwargs,
                                              self.nonlin, self.nonlin_kwargs)

        self.upsample2 = self.upsample(self.filters*2, self.filters, kernel_size=2,
                                       stride=2)
        self.decoder_conv2 = self.basic_block(self.filters*2, self.filters,
                                              self.conv_op, self.conv_kwargs,
                                              self.norm_op, self.norm_op_kwargs,
                                              self.dropout_op, self.dropout_op_kwargs,
                                              self.nonlin, self.nonlin_kwargs)
