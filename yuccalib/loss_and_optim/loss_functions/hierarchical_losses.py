...
from yuccalib.loss_and_optim.utils.LossTree import LossTree
from yuccalib.loss_and_optim.loss_functions.nnUNet_losses import DiceCE
from yuccalib.loss_and_optim.loss_functions.NLL import NLL
...
class HierarchicalLoss(nn.Module):
    def __init__(self, hierarchical_kwargs={}):
        ...
        self.root = LossTree(
            hierarchical_kwargs["rootdict"]
        )  # root represents your hierarchy
            ...
            for branch in level_loss_list:
                ...
                for channel in branch:
                    branch_probs = torch.cat(
                        (branch_probs, probabilities_tosum[:, channel].unsqueeze(1)), 1
                    )
...
class WeightedHierarchicalLoss(HierarchicalLoss):
    def get_loss(self, running_loss, summed_probabilities, target):
        return super().get_loss(running_loss, summed_probabilities, target) * (
            1 / (self.n_level + 1)
        )
                ...
                for channel in branch:
                    ...
                    if previous_branch_for_channel[channel] != branch:
                        summed_probabilities[
                            ...
                            last_summed_probabilities[:, channel : (channel + 1), :, :]
                            + eps
                        )
                        previous_branch_for_channel[channel] = branch
                    else:
...
class ConditionalHierarchicalDiceCELoss(
    HierarchicalDiceCELoss, ConditionalHierarchicalLoss
):
    def __init__(self, hierarchical_kwargs={}):
        super().__init__(hierarchical_kwargs)
