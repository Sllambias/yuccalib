    ...
    if tr is None:
        for _, modname, ispkg in pkgutil.iter_modules(folder):
            if ispkg:
                ...
                tr = recursive_find_python_class([join(folder[0], modname)], class_name,
...
def save_segmentation_from_numpy(seg, outpath, properties, compression=9):
    nib.openers.Opener.default_compresslevel = compression  # slight hacky, but it is what it is
    seg = nib.Nifti1Image(seg, properties['affine'], dtype=np.uint8)
    if properties['reoriented']:
        seg = reorient_nib_image(seg, properties['new_orientation'],
                                 properties['original_orientation'])
    seg.set_qform(properties['qform'])
    seg.set_sform(properties['sform'])
    nib.save(seg, outpath+'.nii.gz', )
    del seg


def save_segmentation_from_logits(logits, outpath, properties, save_softmax=False,
                                  compression=9):
    if save_softmax:
...
def merge_softmax_from_folders(folders:list, outpath, method='sum'):
    ...
    for folder in folders:
        assert cases == subfiles(folder, suffix='.npz', join=False), f"Found unexpected cases. "\
    ...
    for case in cases:
        ...
        save_segmentation_from_numpy(files_for_case, join(outpath, case[:-4]),
